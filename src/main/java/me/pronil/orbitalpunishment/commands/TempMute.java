...
public class TempMute extends BukkitCommand {
    public TempMute() {
        super("TempMute");
    }
    @Override
    public boolean execute(CommandSender sender, String s, String[] args) {
        ...
        if (args.length < 3) {
            sender.sendMessage("§cIncorrect syntax. Correct: /mute <name> <length> <reason>");
            return true;
        }
        ...
        if (targetPlayer != null && targetPlayer.isOnline()) {
            MessageUtils.sendMute(targetPlayer, id, reason.toString(), expiresAt);
        }
